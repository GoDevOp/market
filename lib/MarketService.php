...
class MarketService {
	...
	public function __construct(IConfig $config, IAppManager $appManager, IAppConfig $appConfig) {
		...
		if ($storeUrl === 'https://api.owncloud.com/v1') {
			...
			$this->impl = new Service($appManager, $appConfig, $storeUrl);
		}
	}
	...
	public function updateAvailable($ocsId) {
		return $this->impl->updateAvailable($ocsId);
	}
