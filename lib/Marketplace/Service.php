...
class Service {
	...
	public function __construct(IAppManager $appManager, IAppConfig $appConfig, $storeUrl) {
	...
	public function updateAvailable($appId) {
		$info = $this->getInstalledAppInfo($appId);
		if (is_null($info)) {
	...
	public function getUpdates() {
		...
		foreach ($apps as $app) {
			...
			if (isset($info['id'])) {
				try {
					$appId = $info['id'];
					$newVersion = $this->updateAvailable($appId);
					if ($newVersion) {
						$result[$app] = [
							'version' => $newVersion,
							'id' => $appId
	...
	private function getApps() {
		...
		// read from cache
		if (\OC::$server->getMemCacheFactory()->isAvailable()) {
			$cache = \OC::$server->getMemCacheFactory()->create('ocmp');
